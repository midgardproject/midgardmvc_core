...
class midcom_core_services_authentication_sessionauth implements midcom_core_services_authentication
{
    ...
    {
        $auth_type = midcom_core_midcom::get_instance()->configuration->get('services_authentication_authtype');
        $login_tokens = array
        (
            'login' => $username,
            'authtype' => $auth_type,
    ...
    {
        midcom_core_midcom::get_instance()->authorization->enter_sudo('midcom_core');    
        ...
        $session = new midcom_core_login_session();
        ...
        midcom_core_midcom::get_instance()->authorization->leave_sudo();         
    ...
    {
        $qb = new midgard_query_builder('midcom_core_login_session');
    ...
    public function update_login_session($new_password)
    {
        ...
        $session = new midcom_core_login_session($this->session_cookie->get_session_id());
        $session->password = $pw;
        $session->update();
    }    
    ...
    public function handle_exception(Exception $exception)
    {
            ...
            && isset($_POST['password']))
        {
            if ($this->login($_POST['username'], $_POST['password']))
            {
                ...
                midcom_core_midcom::get_instance()->dispatcher->dispatch();
                return;
            }
        }

        $log_message = str_replace("\n", ' ', $exception->getMessage());
        if (isset(midcom_core_midcom::get_instance()->context->uri))
        {
            $uri = midcom_core_midcom::get_instance()->context->uri;
            $log_message .= " ($uri)";
        }
        midcom_core_midcom::get_instance()->log('midcom_core_services_authentication_sessionauth', $log_message, 'warn');
        
        // Pass some data to the handler
        $data = array();
        $data['message'] = $exception->getMessage();
        $data['exception'] = $exception;
        midcom_core_midcom::get_instance()->context->midcom_core_exceptionhandler = $data;
        
        // Set entry point and disable cache
        midcom_core_midcom::get_instance()->context->template_entry_point = 'midcom-login-form';
        midcom_core_midcom::get_instance()->context->cache_enabled = false;
        
        // Do normal templating
        midcom_core_midcom::get_instance()->templating->template();
        midcom_core_midcom::get_instance()->templating->display();
        exit(0);
    }
